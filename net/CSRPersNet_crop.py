...
pretrain_dict = nn.ModuleList(list(list(models.vgg16_bn(True).children())[0].children())[0:33]).state_dict()

class Frontend(nn.Module):
    def __init__(self, pretrain=True, **kwargs):
        super(Frontend, self).__init__()
        self.pad_conv2d_1 = BasicPerspectiveDilatedConv2D_BN(3, 64, 3, 1, **kwargs)
        self.pad_relu_1 = nn.ReLU(inplace=True)
        self.pad_conv2d_2 = BasicPerspectiveDilatedConv2D_BN(64, 64, 3, 1, **kwargs)
        self.pad_relu_2 = nn.ReLU(inplace=True)
        self.max_pool_1 = nn.MaxPool2d(kernel_size=2)
        
        self.pad_conv2d_3 = BasicPerspectiveDilatedConv2D_BN(64, 128, 3, 1, **kwargs)
        self.pad_relu_3 = nn.ReLU(inplace=True)
        self.pad_conv2d_4 = BasicPerspectiveDilatedConv2D_BN(128, 128, 3, 1, **kwargs)
        self.pad_relu_4 = nn.ReLU(inplace=True)
        self.max_pool_2 = nn.MaxPool2d(kernel_size=2)
        
        self.pad_conv2d_5 = BasicPerspectiveDilatedConv2D_BN(128, 256, 3, 1, **kwargs)
        self.pad_relu_5 = nn.ReLU(inplace=True)
        self.pad_conv2d_6 = BasicPerspectiveDilatedConv2D_BN(256, 256, 3, 1, **kwargs)
        self.pad_relu_6 = nn.ReLU(inplace=True)
        self.pad_conv2d_7 = BasicPerspectiveDilatedConv2D_BN(256, 256, 3, 1, **kwargs)
        self.pad_relu_7 = nn.ReLU(inplace=True)
        self.max_pool_3 = nn.MaxPool2d(kernel_size=2)
        
        self.pad_conv2d_8 = BasicPerspectiveDilatedConv2D_BN(256, 512, 3, 1, **kwargs)
        self.pad_relu_8 = nn.ReLU(inplace=True)
        self.pad_conv2d_9 = BasicPerspectiveDilatedConv2D_BN(512, 512, 3, 1, **kwargs)
        self.pad_relu_9 = nn.ReLU(inplace=True)
        self.pad_conv2d_10 = BasicPerspectiveDilatedConv2D_BN(512, 512, 3, 1, **kwargs)
            ...
            for k,v in pretrain_dict.items():
                ...
                if count != ori_count:
                    if 'weight' in k:
                        ...
                        ori_count += 1
                elif count == ori_count:
                    if 'weight' in k:
                        new_state_dict[name_prefix + '.bn.weight'] = v
                    elif 'bias' in k:
                        new_state_dict[name_prefix + '.bn.bias'] = v
                    elif 'running_mean' in k:
                        new_state_dict[name_prefix + '.bn.running_mean'] = v
                    elif 'running_var' in k:
                        new_state_dict[name_prefix + '.bn.running_var'] = v
                        count += 1
...
class CSRPersNet_BN(nn.Module):
    def __init__(self, load_path=None, is_relu=False, **kwargs):
        super(CSRPersNet_BN, self).__init__()
